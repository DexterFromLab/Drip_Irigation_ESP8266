...
function setVisAutoSec(){
	if($('#autoCont').val() == 0){ 
		$('.autoSec').css('display','none');
		sendEquation('0=Rel0AutoControl');
	}
	else {
		$('.autoSec').css('display','block');
		sendEquation('1=Rel0AutoControl');
	}
}
...
function dispControlMarker(type){
	var tmp = '<br>';
	if($('#controlMark').val().indexOf("Temp") != -1){
		tmp += "<label>Temperatura</label><br>";
		tmp += generateControlSelector(-20,45,100);
	}else if($('#controlMark').val().indexOf("Hum") != -1){
		tmp += "<label>Wilgotność</label><br>";
		tmp += generateControlSelector(0,100,100);		
	}else if($('#controlMark').val().indexOf("Soil") != -1){
		tmp += "<label>Wilgotność gleby</label><br>";
		tmp += generateControlSelector(0,100,100);		
	}
...
function filPossibleControlTypeField(){
	allVirablesNames
	var tmpString ="<option value=0 >Brak</option>";
	for(var i=0;i<allVirablesNames.length;i++){
		tmpString+='<option value="'+allVirablesNames[i]+'">'+allVirablesNames[i]+'</option>'
	}
	$('#controlMark').html(tmpString);
}
